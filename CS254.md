...
- show that it satisfied the coloring property
- Terminates

**Proof**

initially no edge is ocloured an any connected undir graph has an MST -> invariant is satisfied at the beginning of algorithm
...
- Let V1 and V2 be the vertex sets in the parition that caused blue rule to be applied to e

- There must exist a path in T between edgpoints of e and at least one edge e* in the path must connect a vertex from V1 to a vertex in V2
- e* is unclored and we colored edge e
...
Let T be a MST satisfying the invariant belre colouring e

$e\notin T$ the tree satisfyng the invariant after colouring e

other wise $e\in $T

- Let V1 and V2 be the vertex sets in the two subtreed of T obtained after deleting e from T when we applied the red rule to e, there was a ccle C containing e and some other edges with weight less than or equal to e
- $w(e)\geq w(e^*)$ for any $e^*\in C$
- Remove the edge e from the MST
- The new edge will have a lowe weight, and still maintiaing the invariant
...
Suppose by cont. that the algorithm is unable to perfrom either blue or red rule, while some edges of G are uncoloured

Let e be any unclored edge
...
To argue which edge is applied, take a cut of one tree two another, must be one edge which is not colured blue

- take the lightest edge, which can the be coloured blue, which is between the two cutsets
