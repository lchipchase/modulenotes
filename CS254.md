...
- di Graphs - simple
  - V a set of vertices
  - $E\subseteq V \times V$ edges, where E is non - reflexive
...
- Any algorithm that determines if a graph is bipartite that has an input a undirected graph $G=(V,E)$ is bipartite that has as its input a undirected graph $G=(V,E)$ represented as an $n\times n$ adj matrix, has the running time $\Omega(n^2)$
...
- For a given arbitrary graph $G=(V,E)$ represented as $n\times n$ adj matrix A, algorithm ALG
  - Either returns TRUE
  - or Returns FALSE
...
- Prims
  - Simple implementation gives 𝑂 𝑛 2 runtime • Simple implementation using priority queues gives 𝑂(𝑚 log 𝑛) runtime • Advanced data structures (Fibonacci heaps) give the running time of 𝑂(𝑚 + 𝑛 log $n$) 
- Boruvka 
...
- Given a matching $M$ in $G$ a vertex that is not incident to any edge of $M$ is called a free vertex
- For a matching M a path in P in G is called an alternating path if edges in M alternate with edges not in M
...
Because we could switch matching and non-matching edges along the path. This would give a matching $M'=M\oplus P$ - only edges that are in M or P but no both - with larger cardinality

#### $\Longleftarrow$ 

Suppose there is a matching $M^*$ with $|M^*|>M$

- consider the graph $H$ with edge set $M^*\oplus M$
- Each vertex in $H$ can be incident to at most two edges (one from M and one from M*).
- Hence the connected components of $H$ are alternating cycles or alternating paths, each of the cycles will have an equal number of red and blue edges
...
## Matching in Normal graph

As long as you find an augmenting path, augment your matching using this path.

When you arrive at a matching for which no augmenting path exists then you have a maximum matching

### Augmenting in Bipartite

Construct an alternating tree

- Start in a free vertex
- Take all its neighbours - are they free or not
- If not
  - There is a matching edge incident to this
- Repeat this
- Keep building a tree
- Until a free vertex is found, and you have an augmented path

Partition into odd and even vertices:
...
- Case 1:
  - $y$ is a free vertex not contained in $T$
  - We have found an augmented path
- Case 2:
  - $y$ is a matched vertex. Both $y$ and mate[y] are not contained in $T$
- Case 3:
  - $y$ is odd:
  - $y$ us un $T$ as an odd vertex, since an augmenting path from the root to $x$ and then to $y$ can be obtained by starting it directly to $y$
- Case 4:
  - $y$ is an even and already contained in $T$
  - In bipartite graphs, there are no odd length cycle, therefore this case will not happen

This can be constructed in an alternating tree in $O(n+m)$ time

This gives an $O(n(n+m))$ time algorithm to find an augmenting path

- Try to build an alternating tree starting at any single vertex
- Since each matching augmentation increase the size of the matching and since any matching willhave size $\leq \lfloor n/2\rfloor$, we will need at most $\lfloor n/2\rfloor$ augmentations



Giving a faster running  time of $O(\sqrt n\cdot(n+m))$ time

# Fill in notes from lecture 21 - written on paper



M* be the max matching and let M be any matching in G, If the length of the shortest AP wrt M is $k$ then  $M^*|-|M| \leq |V|/k$

#### Hopcroft Karp Algorithm

- Esure the length of the paths grows in each phase
- In each pase construct a maximal set $\Pi$ of disjoin augmenting paths wrt M
  - Denote $M\oplus\Pi = M\oplus(\oplus_{p\in\Pi}P)$ - $\cup$ would also work as all are disjoint
  - The size of the matching by 1 increases at each iteration
  - Increase the size of $M$ by the size of $\Pi$ 

**Lemma**

- let $k$ be the length of the shortes aug path wrt$M$
- Let $\Pi$ be a maximal set of shortes disjoing augmenting paths wrt $M$
- Then the length of the shortest augmenting path wrt $M\oplus \Pi >k$ 



All paths in $\pi$ are of length $k$

Easy way to construct

- Find an ag path P of length k
- Remove it and then find another aug path of length k
- Repeat as long as possible
- The union of these define $\Pi$ 

**Proof**

Consider a shortest path P wrt $M\oplus \Pi$ 

If $P$ doesnt intersect any path from $\Pi$ then its length is $>k$

- Since $P$ doesn;'t intersect any path from $\Pi$ then the maximality implies that $P$ has to be longer than $k$

Hence now we have to consider the case that $P$ intersects $\Pi$

Suppose that P interescts $P_1,...,P_t\in\Pi$ in the given order

Combine the paths to make 
...
- $\Pi$ - – a maximal set of shortest (of length 𝑘) disjoint augmenting paths w.r.t. �

![image-20220228132537262](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220228132537262.png)



- Take any aug path wrt $M\oplus |Pi$

![image-20220228132600870](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220228132600870.png)

- Which goes to this

![image-20220228132628851](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220228132628851.png)

- Now need to create $|\Pi|+1$ augmeingint paths wrt M from $P$ and $\Pi$

![image-20220228132715489](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220228132715489.png)



- With respect to M, the path was not an augmentation, this matching path is after augmentation
- Do this from all of the existing paths

![image-20220228132901973](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220228132901973.png)

- Each path $R_i$ is an augmenting path WRT $M$
- Length of each path $R_i\geq k$ - assuming the path of any augmenting matching is at leat k
- Total length of all paths $R_i$ is shorter than the length of $P$ and all of $P_j$



**Proof Cont.**

- Combine therse paths to crate $t+1$ new aug path $R_i$
- $R_i$ are augmenting paths WRT $M$
- Since any aug path wrt

## Hall's Theorem

- A bipartite graph $G=(L\cup R,E)$ has a perfect matching igg for all set $S\subseteq L.|\Gamma(S)|\geq |S|$ where $\Gamma(S)$ denotes the set of vertices in $R$ that have a neighbour in $S$

**Proof**

$\Rightarrow$

- Because there is a matching, then this is trivially true

$\Leftarrow$

- Induction on the size of $L$

  - When $|L|=1$ this is trivial, because there is one vertex on the left, if thishas at least one edge, there will be a perfect matching

  - For the general case:

    - $|L|>1$

      ​	$\forall S\subseteq L,|\Gamma(S)|\geq |S|$ - implies $G$ has a perfect matching

    - Pick an arbitrary $v\in L$

    - $v$ has at least one neighbour $u\in R$

    - MAtch $v$ to $u$ and find a matching of size $|L|-1$ in the graph $L,R$ by induction



- Remove the vertices

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220301161612707.png" alt="image-20220301161612707" style="zoom:25%;" />

- After removing, this is the state og the graph



<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220301161634958.png" alt="image-20220301161634958" style="zoom:33%;" />



- Using induction, this implies the graph above has the perfect matching
- As we also know, the graph has the same perfect matching, adding $\set{u,v}\in E$ 

- However, we do not have the inductive rule without $\set u$

Suppose there are set $S\subseteq L\backslash \set v$ such that $|\Gamma(S)\backslash \set u |<|S|$  - must be of size $S-1$, otherwise we would have greater than or equal

- Let $Q$ be one such smallest set
- Therefore $|\Gamma(Q)\backslash\set u|<|Q|$, but we also know that $|\Gamma(Q)\geq|Q|$ therefore $Q$ has exactly $|Q|$ neighroubrs in $R$
- Since $\forall U\subset Q |\Gamma(U)|\geq|Q|\implies$ by induction, there is a matching that matches all vertices of $Q$ to vertices from $\Gamma(Q)$

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220301162314160.png" alt="image-20220301162314160" style="zoom:25%;" />
...
We know the following assumption

- For every $A\subseteq L\backslash Q$:
  - $\implies A\cup Q$ has $\geq |A|+|Q|=|A|+|\Gamma(Q)|$ neighbours in $R$
  - $\implies$ there exist  a matching that matches all the vertices of $L\backslash Q$ to $R\backslash \Gamma(Q)$



- Every set $A$ has at least A neighbours in the other half - $A$ is strictly smaller than $L$

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220301162635386.png" alt="image-20220301162635386" style="zoom:25%;" />





$\implies$ there exists a perfect matching between $L$ and $R$

### Application

- For every $D\geq 1$ ever d regular bipartite graph has a perfect matching
- Any such grpah has $d$ edge disjoint perfect matchings

We know the graph is $d$ regular

Therefore, coming out of $S$, there are at least $d|S|$ edges leading into the other side, otherwise, at least one would need a degree higher than $d$

- $|S|\leq|\Gamma(S)|$
- $k$ vertices in $\Gamma(S)$
- $kd\geq d\cdot |S|$
...
1. By halls theorem, every d regular bipartite graph has a perfect matching
2. Remove one perfect matching from the graph to obtain a (𝑑 − 1)–regular bipartite graph. 
3. Repeat … to recursively find 𝑑 edge-disjoint perfect matchings.
...
## Konigs Theorem, vertex cover and matching

- a vertex cover is a set of C vertices such that all edged $e$ of $E$ are incident to at least one vertex of $C$
- There is no edge completely contained in $V\backslash C$

Job normally to find the smallest vertex cover

- The size of any amtching is $\leq$ the size of any vertex cover
  - Given any matching M, a vertex cover C must contain at least one of the endpoints of each edge
- Any vertex cover is at least as large as the maimum size matching



**Theorem**

- for any bipartite graph, the maximum size of a matching is equal to the minimum size of a verex cover

**Proof**

**Claim**

Fix a maximum matching $M$. Let $Q_L$ be all vertices reachable in $G_M$ from free vertices in $L$. Then $C^*=(L\backslash Q_L)\cup(R\cap Q_l)$, moreover $|C^*|=|M|$

- Free vertices in $L$ are in $L\cap Q$
- Taking all the neighbours, then go back to set $L$ using matching edges
- Then continue this and go over to the otherside, until no more vertices are found

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220307131839005.png" alt="image-20220307131839005" style="zoom:25%;" />

**Proof**

- By Contradiction, assume that $C^*$ is not a vertex cover

- Then, there is an edge $e=\set{x,y}\in E$ with $x\in L\cap Q_L,y\in R\backslash Q_L$
- We know that it is not a free vertex, we need to go from a free vertex from it, and this means that we need to reach $x$ from a matching edge, and we know that the start of the matching edge is not reachable from free vertices.
  - We will never reach $y$ therefore will never reach $x$ 
- **Showing no non-matching edge between the two**
- If $x$ is reachable, and there is an edge from $x,y$ then $y$ would also be reachable

**Proof that its $\leq |M|$**

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220307133150994.png" alt="image-20220307133150994" style="zoom:25%;" />

If there is a matching edge in  $R\cap Q_L$, then it is reachable from the free edges

- All free vertices in 𝐿 are in 𝐿 ∩ 𝑄𝐿 
-  All free vertices in 𝑅 are in 𝑅 ∖ 𝑄𝐿 for otherwise we’d get an augmenting path wrt 𝑀, which contradicts to that 𝑀 is maximum 
-  There is no edge from 𝑀 between a vertex 𝑥 ∈ 𝐿 ∖ 𝑄𝐿 and a vertex 𝑦 ∈ 𝑅 ∩ 𝑄𝐿 , for otherwise 𝑥 would be in 𝑄𝐿 
- Hence every vertex in 𝐶 ∗ is matched in 𝑀 and the corresponding matching edges are distinct

Therefore all matching edges are of this sort

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220307133304233.png" alt="image-20220307133304233" style="zoom:25%;" />





- None of these are free vertices, so they are incident to a matching edge, and all matching edges are distinct from the bottom grey
- This immediately yields Konigs theorem



# Weighted Bipartite Matching

- Input undir bipartite graph $G=(L\cup R,E)$
...
- Suppose we have $n$ resources to which we want to assign $n$ tasks on a one-to-one bases. Suppose also that we know the cost of assigning a given resource to a given task. We wish to find an optimal assignment-one which minimises total cost
  - Easy to extend to the task of maximising total cost
  - If representing in a table, in each solumn, you will have one assignment

## Algorithm Outline

- Introduce a node weighting $x^{\rightarrow}$

- For any node $v\in V$, let $x_v\geq 0$ denote the weight of node $v$

  - Suppose that the node wights dominate the edge weights in the following sense:

    - $x_u +x_v\geq w(e)$ holds for ever edge $e=(u,v)$

    Let $H(x^{\rightarrow})$ denote the subgraph of $G$ that only contains edge that are tight $wrt$ the node weighting - if 

  - Try and compute a perfect matching in the subgraph $H(x)$

    - If you succeed the wiehg of your matching is equal to the total node weight $X:=\sum_{v\in V} x_vv$ 

- All edges not drawn have weight 0

The best weighting have weight $0$

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220307134940879.png" alt="image-20220307134940879" style="zoom:25%;" />





- The tight edges give a perfect matching

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220307135235787.png" alt="image-20220307135235787" style="zoom:25%;" />

- any edges which have are not shown have weight 0, therefore the addition of those edges to the graph is trivial.

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220308161758411.png" alt="image-20220308161758411" style="zoom:25%;" />
...
- Find a node weight so that $H(x^\rightarrow)$ has a perfect matching
- If $H(x^\rightarrow)$ does not have a perfect matching, update the weights

**If no perfect matching is found**

**Halls theorem**

- Guarantees that there is a set $S\subseteq L$ with $|\Gamma(S)|<|S|$ where $\Gamma$ denotes the neighbourhood wrt the subgraph $H(x^\rightarrow)$

- Re-weight such that:
  - The total weight assigned to nodes decreases
  - The weight functionstill dominates the weights
  - Size of the maximum matching n $H$ goes up

### Changing node weights

Increase node-weights in Γ(𝑆) by +𝛿, 

and decrease node-weights in 𝑆 by −𝛿.

- Total node-weight decreases
- Only edges from $S$ to $R\backslash \Gamma(S)$ decrease in their weight
- None of these edges are tight otherwise this would be contained in $H(x^\rightarrow)$ would go between $S$ and $\Gamma(S)$ 
- We can do this decrement for small enough $\delta$ until a new edge gets tight 

$\implies$ node wights still domainate

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220308163342871.png" alt="image-20220308163342871" style="zoom:25%;" />

### Number of Iterations

- One re-weighting increases the number of edges out of $S\geq 1$
- Assume that we have a max matching that sturates the set $\Gamma(S)$ in the sense that every node in $\Gamma(S)$ is matched to a node in $S$
  - Show that can always find $S$ and a matching such that this holds
- MAtching is still contained in the new graph, because all edges either go between $\Gamma(S)$ or between $L\backslash S$ and $R\backslash \Gamma(S)$
- re-weighting does not decrease the size of a maximum matching in the tight subgraph.
- We will show that after at most 𝑛 re-weighting steps the size of the maximum matching can be increased by finding an augmenting path.
- Gives polynomial time

### Finding $S$

- For a bipartite graph $H(x^\rightarrow)=(L\cup R,R)$ and for a amtching $M$ define a digraph $G$

- 

### Analysis

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220308164903908.png" alt="image-20220308164903908" style="zoom:25%;" />



- After at most $n$ steps an augmenting path will be found
- A re-wrighting can be trivially perfomed in $O(n^2)$
- An augmentation takes at most $O(n)$ times
- Total running time of $O(n^4)$
...
# Write about lecture 27

# Maximum matching In General Graphs - redo this lecture

- Given a matching $M$ and a graph $G$, a vertex that is not incident to any edge of $M$ is called a fre vertex
- For a matching $M$, a path $P$ in $G$  is an alternating path if edges in $M$ alternate with edges not in$M$
- An 

## Finding an aug path

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131041332.png" alt="image-20220314131041332" style="zoom:33%;" />



- partition into odd and even vertices

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131110997.png" alt="image-20220314131110997" style="zoom:25%;" />

We end up with a vertex $x$

- **With $y$ outside the tree and a free vertex**

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131153615.png" alt="image-20220314131153615" style="zoom:25%;" />

- - Found an augmenting path

- **With $y$ outside the tree and not a free vertex**
  - 𝒚 is a matched vertex. both 𝒚 and mate[𝒚] are not contained in �

**Y is inthe tree**

![image-20220314131319237](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131319237.png)

**Case 4**

![image-20220314131351038](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131351038.png)

- This odd length cycle is called a blossom
  - Let $w$ denote the LCA of x,y
  - The odd cycle indicued by the tree 
  - <img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131447930.png" alt="image-20220314131447930" style="zoom:25%;" />
...
When during the alternating tree construction we discover a blossom 𝐵, we do the following: • Replace the graph 𝐺 = (𝑉, 𝐸) by 𝐺 ′ = 𝐺/𝐵 • 𝐺 ′ = 𝐺/𝐵 is obtained from 𝐺 by first deleting all vertices in 𝐵 and all edges incident to these vertices, and then adding a new (pseudo-)vertex 𝑏. The new vertex 𝑏 is connected to all vertices in 𝑉 ∖ 𝐵 that had at least one edge to a vertex from 𝐵.

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131536402.png" alt="image-20220314131536402" style="zoom:25%;" />

- Dashed edges are not examined yet
- Solid edges are part of the tree but not mathcing edges
- The base $w$ is an even node in the tree
- The paremt edge of $w$ is the only matching edge leaving $b$

Want to convert above into:

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131803008.png" alt="image-20220314131803008" style="zoom:25%;" />

### Algorithm

![image-20220314131948493](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314131948493.png)

# Matching in planar graphs



Using algorithm we have seen before

- Max matching found in $O(nm)$
- Since in simple planar graphs have an order of $n$ edges, this can be done in $O(n^2)$

## Planar Separator Theorem

- rEcursive algorithm
  - Split the graph into two parts
  - Solve the problem recursively in each part
  - Combine the solutions
- Take a 2d grid of size $\sqrt n\times \sqrt n$
  - Can split it into two cuts of $\Theta(\sqrt n)$

<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314133441893.png" alt="image-20220314133441893" style="zoom:25%;" />

- Let $G=(V,E)$ be an SPG, partition $V$ into three subsets $A,B,S\subseteq V$
- <img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314133538940.png" alt="image-20220314133538940" style="zoom:25%;" />



<img src="C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314133644590.png" alt="image-20220314133644590" style="zoom:25%;" />



Since ABS are small, can solve the amtching problem recursively 

- Find the maximum matching subset of G induced by A
- Find the maximum matching subset of G induced by B
- insert the vertices in $S$

After the first two recursive calls, we found a maximum matching in the subgraph of $G$ induced by vertices from $A\cup B$

Inserting any new vertex from $S$ corresponds to an augmenting path

Add a single vertex corresponding to the following

- We found a maximum  matching $M$ in graph $G'$
- Want to find maximum matching in a graph $G^*=G'\cup \set v$
- This is the problem of finding in an aug path in $G^*$ wrt $M$ starting at $v$
  - Corresponds to finding an aug path WRT $m$
...
![image-20220314134332098](C:\Users\leonc\Documents\Warwick\CSY2\modulenotes\CS254.assets\image-20220314134332098.png)
